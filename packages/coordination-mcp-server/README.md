# @hivemind/coordination-mcp-server

The coordination layer for **Hivemind** — collision avoidance + shared memory for multiple people coding the same repo with AI agents (Claude Code / Codex). It implements the MVP slice of [`docs/design/coordination-mcp.md`](../../docs/design/coordination-mcp.md).
...
- **Claims with fence tokens.** `hive_claim` grants an exclusive lease on a region (file or `path#Symbol`) and returns a monotonic `fence`. The fence is the spine: the write path validates it so a lapsed holder can't clobber a new one (invariant I12).
...
- **Presence** (`hive_announce` / `hive_whos_editing`) — ephemeral who's-editing-what awareness.
- **Decisions bus** (`hive_post_decision` / `hive_get_decisions`) — an append-only, supersede-chained shared memory scoped to repo/file/region so teammates' agents pick up the few decisions that matter.
...
Endpoints:
- `POST /mcp` — the MCP (Streamable HTTP) endpoint agents connect to (tools `hive_*`).
...
| Tool | Purpose |
|---|---|
| `hive_resolve_region` | (repo, path[, symbol]) → stable regionId |
| `hive_register` | declare an actor as agent/human |
| `hive_claim` | claim a region before editing → `{fence, claim_id, ttl}` or `REGION_CLAIMED` |
| `hive_release` | release a held claim (fence-checked) |
| `hive_heartbeat` | extend a lease (requires advancing progress) |
| `hive_whos_editing` | active claims + presence |
| `hive_announce` | publish presence |
| `hive_post_decision` | append a scoped, supersede-chained decision |
| `hive_get_decisions` | retrieve relevant chain-head decisions |
